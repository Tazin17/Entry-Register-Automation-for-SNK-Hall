...
const NewPostForm = ({ isPostFormOpen, setIsPostFormOpen }) => {
  ...
  const handlePost = async () => {
    try {
      setLoading(true);
      console.log(title,date,description,postContent);
      const formData = new FormData();
      formData.append("date", date);  // Include title
      formData.append("date", date);    // Include date
  ...
  return (
    <Card className="w-[940px] h-[580px] ml-15 bg-[rgb(40,85,85)]">
        ...
        <div className="flex space-x-4">
          ...
          <Dialog open={isPostFormOpen} onOpenChange={setIsPostFormOpen}>
            <DialogTrigger className="w-full">
              <Input
                placeholder={`Write your application here, ${user?.username}`}
                readOnly
                className="cursor-pointer rounded-full h-12  dark:bg-[rgb(58,59,60)] placeholder:text-gray-200 dark:placeholder:text-gray-200  "
              />
            ...
            <DialogContent className="sm:max-w-[525px] max-h-[80vh] overflow-y-auto">
              ...
              {/* New fields for title, date, and description */}
              <Input
                placeholder="Date"
                value={date}
              ...
              /> */}
              <Textarea
                placeholder={`Write your notice here ${user?.username}`}
